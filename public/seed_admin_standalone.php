...
// Password hashing function (same as CodeIgniter)
...
<body>
    <div class="container">
        ...
        if ($result->num_rows > 0) {
            ...
            echo '<tr><td>Role</td><td>' . htmlspecialchars($existingAdmin['role']) . '</td></tr>';
            ...
            try {
                ...
                $stmt = $mysqli->prepare("INSERT INTO users (username, email, password, first_name, last_name, role, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param('ssssssss', $username, $email, $password, $first_name, $last_name, $role, $status, $created_at);
                
                if ($stmt->execute()) {
                    $admin_id = $mysqli->insert_id;
                    
                    // Fetch created admin
                    $stmt = $mysqli->prepare("SELECT id, username, email, role, status, created_at FROM users WHERE id = ?");
                    $stmt->bind_param('i', $admin_id);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    $createdAdmin = $result->fetch_assoc();
                    
                    echo '<div class="success">✓ Admin user created successfully!</div>';
                    echo '<table>';
                    echo '<tr><th>Field</th><th>Value</th></tr>';
                    echo '<tr><td>ID</td><td>' . $createdAdmin['id'] . '</td></tr>';
                    echo '<tr><td>Username</td><td>' . htmlspecialchars($createdAdmin['username']) . '</td></tr>';
                    echo '<tr><td>Email</td><td>' . htmlspecialchars($createdAdmin['email']) . '</td></tr>';
                    echo '<tr><td>Role</td><td>' . htmlspecialchars($createdAdmin['role']) . '</td></tr>';
                    echo '<tr><td>Status</td><td>' . htmlspecialchars($createdAdmin['status']) . '</td></tr>';
        ...
        <div class="warning">
            ...
            <p>File to delete: <code>/public/seed_admin_standalone.php</code></p>
        </div>
